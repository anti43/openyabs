 ...
 */
public class ObjectsCalendarDayChooser extends JPanel implements ActionListener, KeyListener, FocusListener {

    private static final long serialVersionUID = 5876398337018781820L;
    protected ObjectsCalendarButton[] days;
    protected JButton[] weeks;
    protected ObjectsCalendarButton selectedDay;
    ...
    private static ArrayList<DatabaseObject> events;
    private static ArrayList<DatabaseObject> processedEvents = new ArrayList<DatabaseObject>();
    private static ObjectsCalendarDayChooser icke;
     ...
     * @return
     */
    public static ObjectsCalendarDayChooser instanceOf(Context context) {
        if (icke == null) {
            icke = new ObjectsCalendarDayChooser(context);
        }
        return icke;
    }
...
//        }
     ...
     */
    private ObjectsCalendarDayChooser(Context c) {
        this(true, c);
    }
     ...
     */
    private ObjectsCalendarDayChooser(boolean weekOfYearVisible, Context c) {
        this.context = c;
        setName("JDayChooser");
        setBackground(Color.blue);
        this.weekOfYearVisible = weekOfYearVisible;
        locale = Locale.getDefault();
        days = new ObjectsCalendarButton[49];
        selectedDay = null;
        ...
        for (int y = 0; y < 7; y++) {
            for (int x = 0; x < 7; x++) {
                ...
                if (y == 0) {
                    ...
                    days[index] = new ObjectsCalendarButton(new Date()) {
...
//                days[index].setBorderPainted(false);
     ...
     * @param list
     */
    protected void drawDays(ArrayList<DatabaseObject> list) {

        if (list == null) {
            try {
                list = DatabaseObject.getObjects(context, new vTimeframe(DateConverter.getStartOfMonth(calendar.getTime()), DateConverter.getEndOfMonth(calendar.getTime())));
        ...
        while (day.before(firstDayInNextMonth)) {
            ...
            if (events != null) {
                for (int j = 0; j < events.size(); j++) {
                    DatabaseObject item = events.get(j);
//                  Log.Debug(this, "Checking date: " + schedule.getDate() + " against " + day);
                    if (new vTimeframe(item.__getDateadded(), DateConverter.getEndOfDay(item.__getDateadded())).contains(day)) {
//                        Log.Debug(this, "Found date: " + schedule.getDate());
                        days[i + n + 7].setIcon(item.getIcon());
                        try {
                            days[i + n + 7].setToolTipText(Messages.ACTION_OPEN + " " + item.__getCName());
                            listeners = days[i + n + 7].getActionListeners();
                            for (int k = 0; k < listeners.length; k++) {
                                ActionListener li = listeners[k];
                                days[i + n + 7].removeActionListener(li);
                            ...
                            days[i + n + 7].addActionListener(new ActionListener() {
                                ...
                                public void actionPerformed(ActionEvent e) {
                                    try {
                                        ...
                                        MPView.identifierView.addTab(
                                                new GeneralListPanel(
                                                DatabaseObject.getObjects(
                                                context, new vTimeframe(tday,
                                                DateConverter.getEndOfDay(tday)))),
                                                DateConverter.getDefDateString(tday));
                                    } catch (NodataFoundException ex) {
                                    }
                                }
                            });
...
//                            DatabaseObject item1 = processedEvents.get(k);
//                            if (new vTimeframe(item1.__getDateadded(), DateConverter.getEndOfDay(item1.__getDateadded())).contains(day)) {
//                                found = true;
//                            }
//                        }
//                        if (!found) {
...
//    }
     ...
     */
    class DecoratorButton extends ObjectsCalendarButton {
