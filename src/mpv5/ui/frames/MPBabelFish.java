...
//        MPTableModel mpdel = DataModelUtils.getModelCopy(data);
        dialog.saveFile(ArrayUtilities.tableModelToFile(data, new int[]{0, 1}, "=", String.valueOf(languages.getSelectedItem()), "properties"));
//        data.setModel(mpdel);
//        setLanguageSelection();
            ...
            for (int i = 0; i < originallanguage.length; i++) {
                ...
                try {
                    for (int j = 0; j < imported.length; j++) {
                        ...
                        if (split.length == 2) {
                            if (split[0].trim().equals(component)) {
                                originallanguage[i] = split[1].trim();
                                Log.Debug(split[0].trim(), originallanguage[i].trim());
                                mpv5.YabsViewProxy.instance().setProgressValue(i + 1);
                                progress.setValue(i + 1);
                            }
                        }
